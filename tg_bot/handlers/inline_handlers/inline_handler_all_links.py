...
@button_link_router.callback_query(F.data == 'social_links')
...
@button_link_router.callback_query(F.data == 'links_menu')
async def process_button_links_menu_press(callback: CallbackQuery):
    user_id = callback.from_user.id

    try:
        await callback.message.answer(
            text='Ссылки на наши социальные сети:',
            reply_markup=links_menu_inline
        )
        await callback.message.delete()
        await callback.answer()
        logger.debug(f"Обработка запроса на ссылки завершена для пользователя с ID: {user_id}")
